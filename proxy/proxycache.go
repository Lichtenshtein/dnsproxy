...
func (p *Proxy) cacheResp(d *DNSContext) {
	...
	if !p.EnableEDNSClientSubnet {
		dctxCache.set(d.Res, d.Upstream, d.IsInternalPrefetch, p.logger, d.IsRefresh)
	...
	case ecs != nil && d.ReqECS != nil:
		...
		dctxCache.setWithSubnet(d.Res, d.Upstream, ecs, d.IsInternalPrefetch, p.logger, d.IsRefresh)
	case d.ReqECS != nil:
		// Cache the response for all subnets since the server doesn't support
		// EDNS Client Subnet option.
		dctxCache.setWithSubnet(d.Res, d.Upstream, &net.IPNet{IP: nil, Mask: nil}, d.IsInternalPrefetch, p.logger, d.IsRefresh)
	default:
		dctxCache.set(d.Res, d.Upstream, d.IsInternalPrefetch, p.logger, d.IsRefresh)
	}
}
